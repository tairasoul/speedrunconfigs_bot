...
const { gh_token, author } = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "config.json"), 'utf8'));
    ...
    for (const directory of sorted) {
        if (fs.existsSync(`${git_repo_path}/${directory}`)) {
            ...
            for (const build of builds) {
                ...
                if (!fs.existsSync(`${git_repo_path}/${directory}/Builds`))
                    fs.mkdirSync(`${git_repo_path}/${directory}/Builds`);
...
function removeEmptyDirectoriesUpToRoot(directory, rootDirectory) {
...
async function FileHandle(username, build, file, data, additionalPath = "") {
...
async function RemoveFile(username, build, file, additionalPath = "", isBuildRemoval = false) {
    const dir = path.join(git_repo_path, username, build, additionalPath);
    if (!fs.existsSync(dir))
        return;
    const filePath = path.join(dir, file);
...
export default class GitHandler {
    ...
    async handleFile(username, build, file, data, additionalPath = "") {
        await this.update();
        await FileHandle(username, build, file, data, additionalPath);
        await updateRepo();
    }
    async removeFile(username, build, file, additionalPath = "") {
        await this.update();
